...
$query = mysqli_query($connection,"SELECT * FROM serverip") or die ("Error fetch server IP!");
$ip = mysqli_fetch_array($query);
if (!file_exists("script\ip")){
	$ipFile = fopen("script\ip","w") or die ("Unable to open the file!");
	fwrite($ipFile,$ip[0]);
	fclose($ipFile);
}
if(file_exists("script\ip")){
if(isset($_SESSION["USERNAME"])) {
	system("script\connection_handler.bat",$errorlevel);
	if($errorlevel!=1) {
		system("del script\ip");
		$query = mysqli_query($connection, "DELETE FROM serverip WHERE ip='$ip[0]'");
		die ("Connection lost!");
	}
}
}
